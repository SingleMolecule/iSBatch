...
public class getFirstOfEach implements PlugIn{
	  	...
	  	if( choice == -1){
	  		...
	  		for(int row=0; row<table.getCounter(); row++){
	  			...
	  			if(currentChannel.equalsIgnoreCase(channel)){
	  				...
	  				averageImp = IJ.createImage(channel+"_FirstOfEach", tempImp.getWidth(), tempImp.getHeight(), StackSize, 16);
	  				tempImp.close();
	  			}
	  		}
	  		...
	  		for(int row=0; row<table.getCounter(); row++){
	  			...
	  			if(currentChannel.equalsIgnoreCase(items[choice])){
	  				...
	  				stack.setProcessor(processor, currentStack);
