...
public class Detect_Peaks_iSB implements PlugIn {
	...
	private static void CalculatePeaksonChannel(String labelsName) throws IOException {
	  			...
	  			//Loop in the ImagePlus
	  				...
	  				for(int stackPosition = 1; stackPosition<=stackSize; stackPosition++)
	  				{
	  				 ...
	  				 for (String label : table.keySet()) {
	  				   ...
	  				   //make the name
	  				   String CellCode = ExperimentIndexNumber + "-" + roi.getName();
	  				  // TotalIntensity.addValue(CellCode, RawIntensity);
	  				   ...
	  				   CellIntensity.addValue("ExperimentIndex", ExperimentIndexNumber);
	  		...
	  		String Destination2 = StatsFolder + File.separator + labelsName +".Peaks.LocalizedNonFiltered"+"[RawPeakList]SNR" + SignalToNoiseTreshold +".csv";
	  		PeakData.saveAs(Destination2);
	  		CellIntensity.saveAs(StatsFolder + File.separator + labelsName +"CellIntensity"+ ".csv");
	  		TotalIntensity.saveAs(StatsFolder + File.separator + labelsName +"TotalIntensity.csv");
