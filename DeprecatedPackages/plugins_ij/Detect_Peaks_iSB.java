...
public class Detect_Peaks_iSB implements PlugIn {
	...
	static ResultsTable CellIntensity = new ResultsTable();
	static ResultsTable TotalIntensity = new ResultsTable();
	...
	private static void CalculatePeaksonChannel(String labelsName)
			throws IOException {

		System.out.println("Detecting peaks");
		CellIntensity.incrementCounter();

		// List<String> listchannel = getSubFileList(table,
		// "Channel",labelsName, "SaveToPath");
...

		{
			...
			if (currentChannel.equalsIgnoreCase(labelsName)) {
				...
				int ExperimentIndexNumber = (int) table.getValue(
						"ExperimentIndex", row);
				...
				for (int stackPosition = 1; stackPosition <= stackSize; stackPosition++) {
					...
					TotalIntensity.incrementCounter();
					...
					for (String label : table.keySet()) {
						...
						ImageProcessor ip3 = stack.getProcessor(stackPosition);
						ip3.setRoi(roi);
						ImageStatistics stats = ip3.getStatistics();

						double RawIntensity = (stats.pixelCount * stats.mean);
						// make the name
						String CellCode = ExperimentIndexNumber + "-"
								+ roi.getName();
						// TotalIntensity.addValue(CellCode, RawIntensity);

						TotalIntensity.setValue(CellCode, row, RawIntensity);

						CellIntensity.addValue("BFSlice", Folder);
						CellIntensity.addValue("ExperimentIndex",
								ExperimentIndexNumber);
						CellIntensity.addValue("Area", stats.area);
						CellIntensity.addValue("slice", stackPosition);
						CellIntensity.addValue("Mean", stats.mean);
						CellIntensity.addValue("px", stats.pixelCount);

						DiscoidalAveragingFilter filter1 = new DiscoidalAveragingFilter(
								ip2.getWidth(), 1, 3);

						/*
						 * Workaround. Get full intensity in a cell
						 */
						// PeakFinder peakFinder = new PeakFinder(true, filter,
						// 0, PeakFinderThreshold, 3);
						// PeakFinder peakFinder2 = new PeakFinder(true,
						// filter1, 0, PeakFinderThreshold, 3);
						PeakFinder peakFinder2 = new PeakFinder(false, filter1,
								SignalToNoiseTreshold, PeakFinderThreshold, 5);

						// ArrayList<Point> positions =
						// peakFinder.findPeaks(ip);
						...
						for (int j = 0; j < positions.size(); j++) {
							...
							PeakData.addValue(
									"error_fwhm",
									Math.sqrt(errorFwhmx * errorFwhmx
											+ errorFwhmy * errorFwhmy) / 2);

							// table.addValue("z", zScale * (fwhmx - fwhmy));
							// table.addValue("error_z", zScale *
		...
		String Destination2 = StatsFolder + File.separator + labelsName
				+ ".Peaks.LocalizedNonFiltered" + "[RawPeakList]SNR"
				+ SignalToNoiseTreshold + ".csv";
		PeakData.saveAs(Destination2);
		CellIntensity.saveAs(StatsFolder + File.separator + labelsName
				+ "CellIntensity" + ".csv");
		TotalIntensity.saveAs(StatsFolder + File.separator + labelsName
				+ "TotalIntensity.csv");
		...
		if (PeakData.getCounter() == 0) { // table is empty
			IJ.log("No peaks detected for channel " + labelsName
					+ " using threshold value: " + PeakFinderThreshold
					+ ". No table will be saved!");

		}

		else {

			String Destination = StatsFolder + File.separator + labelsName
					+ ".Peaks.LocalizedNonFiltered" + "[" + PeakFinderThreshold
					+ "]" + ".csv";
			PeakData.saveAs(Destination);
			IJ.log("Saving full table peak for channel " + labelsName
					+ " using threshold value: " + PeakFinderThreshold);
			...
			String Destination1 = StatsFolder + File.separator + labelsName
					+ ".Peaks.LocalizedFiltered" + "[" + PeakFinderThreshold
					+ "]" + ".csv";
			IJ.log("Saving filtered table peak for channel " + labelsName
					+ " using threshold value: " + PeakFinderThreshold);
	...
	private static void addLocalization(ResultsTable peakData,
		...
		for (int row = 0; row < peakData.getCounter(); row++) {
			...
			if (closestMesh != null) {
				...
				cellPeakPositions.Point projection = closestMesh
						.projectionOf(p);

				// peakData.incrementCounter();
				peakData.setValue("L", row, projection.x);
				peakData.setValue("D", row, projection.y);
				peakData.setValue("L_normalized", row, projection.x
						/ closestMesh.getTotalLength());
