...
//	}
	...
	public void run(String arg0) {
		...
		for (int i = 0; i < size; i++) {
			...
			File f = new File(currentNode.getOutputFolder()+ File.separator + "Diffusion");
			f.mkdirs();
			
			IJ.run(imp, "Peak Fitter", "use_discoidal_averaging_filter inner_radius=1 outer_radius=3 threshold=6 threshold_value=0 minimum_distance=8 fit_radius=4 max_error_baseline=5000 max_error_height=5000 max_error_x=1 max_error_y=1 max_error_sigma_x=1 max_error_sigma_y=1 stack");
			
			IJ.run("Particle Tracker", "slice_to_look_ahead=1 max_step_size=5 show_trajectories");
