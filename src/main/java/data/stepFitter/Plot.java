...
public class Plot extends Component implements Runnable {
		...
		addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseDragged(MouseEvent e) {
				synchronized (lock) {
					...
					double sy = (getHeight() - bottomMargin)
			...
			public void mouseMoved(MouseEvent e) {
				...
				double y1 = ((getHeight() - bottomMargin) - y) / sy
						+ visibleBounds.y;

				double minDistance = Math.sqrt(400 / (sx * sx) + 400
						/ (sy * sy));
			...
			public void mouseClicked(MouseEvent e) {
				synchronized (lock) {
					if (e.getClickCount() == 2) {
						targetedVisibleBounds = (Rectangle2D.Double) plotBounds
								.clone();
					} else {
						double sx = (getWidth() - leftMargin)
								/ visibleBounds.width;
						double sy = (getHeight() - bottomMargin)
								/ visibleBounds.height;

						// mouse position
						double mx = (e.getX() - leftMargin) / sx
								+ visibleBounds.x;
						double my = ((getHeight() - bottomMargin) - e.getY())
								/ sy + visibleBounds.y;
						...
						targetedVisibleBounds.x = mx
								- targetedVisibleBounds.width / 2;
						targetedVisibleBounds.y = my
								- targetedVisibleBounds.height / 2;
					}
	...
	public void run() {
		...
		while (true) {
			...
			if (Math.abs(dx * sx) < 1 && Math.abs(dy * sy) < 1
					&& Math.abs(dw * sx) < 1 && Math.abs(dh * sy) < 1) {
				synchronized (lock) {
					try {
						lock.wait();
	...
	private final float getStepSize(double range, int steps) {
		...
		double magnitude = Math.pow(10, Math.floor(Math.log10(step))); // e.g.
																		// 0.001
	...
	public void paint(Graphics g) {
		...
		for (double x = Math.ceil(visibleBounds.x / stepx) * stepx; x <= visibleBounds.x
				+ visibleBounds.width; x += stepx) {
		...
		for (double y = Math.ceil(visibleBounds.y / stepy) * stepy; y <= visibleBounds.y
				+ visibleBounds.height; y += stepy) {
